...
class _HotMoviesPageState extends State<HotMoviesPage> {
  ...
  void _openDetail(Map<String, dynamic> movie) {
    ...
    else if (t == 'tay anh giữ một vì sao' || t == 'tay anh giu mot vi sao')
      page = const TayAnhGiuMotViSaoDetailPage();
    else if (t == 'tee yod')
      page = const TeeYodDetailPage();
    else if (t == 'tử chiến trên không' || t == 'tu chien tren khong')
      page = const TuChienTrenKhongDetailPage();
    else if (t == 'avatar 3' || t == 'avatar3')
    ...
    else if (t == 'năm của anh, ngày của em' || t == 'nam cua anh, ngay cua em')
      page = const NamCuaAnhNgayCuaEmDetailPage();
    else if (t == 'gió vẫn thổi' || t == 'gio van thoi')
      page = const GioVanThoiDetailPage();
    else if (t == 'cải mả' || t == 'cai ma')
      page = const CaiMaDetailPage();
    else if (t == 'cục vàng của ngoại' || t == 'cuc vang cua ngoai')
      page = const CucVangCuaNgoaiDetailPage();
    else if (t == 'good boy - chó cưng đừng sợ' ||
        t == 'good boy - cho cung dung so')
      page = const GoodBoyDetailPage();
    else if (t == 'tớ và roboco: siêu cấp đa vũ trụ' ||
        t == 'to va roboco: sieu cap da vu tru')
    ...
    if (page != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Chưa có trang chi tiết cho phim này.')));
    }
  }
      ...
      onSelected: (v) {
        if (v == 'logout') {
          ...
          ScaffoldMessenger.of(context)
              .showSnackBar(const SnackBar(content: Text('Đi đến Hồ sơ')));
        } else if (v == 'tickets') {
          ScaffoldMessenger.of(context)
              .showSnackBar(const SnackBar(content: Text('Đi đến Vé của tôi')));
        }
      },
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          _SortChip(
              label: 'Hot',
              selected: value == _HotSort.hot,
              onTap: () => onChanged(_HotSort.hot),
              icon: Icons.local_fire_department_outlined),
          const SizedBox(width: 8),
          _SortChip(
              label: 'Lượt xem',
              selected: value == _HotSort.views,
              onTap: () => onChanged(_HotSort.views),
              icon: Icons.visibility_outlined),
          const SizedBox(width: 8),
          _SortChip(
              label: 'Yêu thích',
              selected: value == _HotSort.likes,
              onTap: () => onChanged(_HotSort.likes),
              icon: Icons.favorite_border),
          const SizedBox(width: 8),
          _SortChip(
              label: 'Đánh giá',
              selected: value == _HotSort.rating,
              onTap: () => onChanged(_HotSort.rating),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(_fmt(movie['views'] as int),
                            style: const TextStyle(fontSize: 12)),
                        const SizedBox(width: 10),
                        const Icon(Icons.favorite_border, size: 14),
                        const SizedBox(width: 4),
                        Text(_fmt(movie['likes'] as int),
                            style: const TextStyle(fontSize: 12)),
                      ],
                    ),
                    const Spacer(),
