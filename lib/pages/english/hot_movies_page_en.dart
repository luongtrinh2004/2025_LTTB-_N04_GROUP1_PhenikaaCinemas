...
class _HotMoviesPageEnState extends State<HotMoviesPageEn> {
  ...
  List<Map<String, dynamic>> _sortedEn() {
    ...
    list.sort((a,b){
      switch(_sortBy){
        case _HotSortEn.views:  return (b['views'] as int).compareTo(a['views'] as int);
        case _HotSortEn.likes:  return (b['likes'] as int).compareTo(a['likes'] as int);
        case _HotSortEn.rating: return (b['rating'] as num).compareTo(a['rating'] as num);
        case _HotSortEn.hot: default: return _hotScoreEn(b).compareTo(_hotScoreEn(a));
      }
    });
    return list;
  }
  ...
  void _openDetailEn(Map<String, dynamic> movie) {
    ...
    if(page!=null){ Navigator.push(context, MaterialPageRoute(builder:(_)=>page!)); }
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No detail page available for this movie yet.')));
    }
  }
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Row(
                  ...
                  children: const [
                    Text('Top Trending 🔥', style: TextStyle(fontWeight: FontWeight.w700)),
                  ],
                ),
              ),
            ),
    ...
    return PopupMenuButton<String>(
      onSelected: (v) {
        if (v == 'logout') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Go to Profile')));
        } else if (v == 'tickets') {
...
class _SortRowEn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Row(
        children: const [
          _SortChipEn(label:'Hot',    icon:Icons.local_fire_department_outlined,  type:_HotSortEn.hot),
          SizedBox(width: 8),
          _SortChipEn(label:'Views',  icon:Icons.visibility_outlined,            type:_HotSortEn.views),
          SizedBox(width: 8),
          _SortChipEn(label:'Likes',  icon:Icons.favorite_border,                type:_HotSortEn.likes),
          SizedBox(width: 8),
          _SortChipEn(label:'Rating', icon:Icons.star_border,                    type:_HotSortEn.rating),
        ],
      ),
    );
  }
...
class _SortChipEn extends StatelessWidget {
  final String label; final IconData icon; final _HotSortEn type;
  const _SortChipEn({required this.label, required this.icon, required this.type});
  @override
  Widget build(BuildContext context) {
...
class _MovieCardEn extends StatelessWidget {
  ...
  const _MovieCardEn({required this.movie, required this.rank, required this.hotScore, this.onTap});
            ...
            Expanded(
              child: Stack(
                children: [
                  ...
                  Positioned(bottom: 8, right: 8, child: _RatingBadgeEn(rating: (movie['rating'] as num).toDouble())),
                ],
              ),
            ),
            SizedBox(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(_fmtEn(movie['views'] as int), style: const TextStyle(fontSize: 12)),
                        const SizedBox(width: 10),
                        const Icon(Icons.favorite_border, size: 14),
                        const SizedBox(width: 4),
                        Text(_fmtEn(movie['likes'] as int), style: const TextStyle(fontSize: 12)),
                      ],
                    ),
                    const Spacer(),
                    ClipRRect(
